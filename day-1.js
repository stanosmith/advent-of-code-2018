...
const resultingFrequency = frequencyChanges.reduce(sum, startingFrequency);
let allFrequencyResults = checkFrequency(frequencyChanges, [0]);
let firstRepeatedFrequency = undefined;
...
function checkFrequency(changes, allResults) {
  return changes.reduce((accumulator, value) => {
    const currentFrequency = accumulator[accumulator.length - 1];
    const result = sum(currentFrequency, value);
...
while (typeof firstRepeatedFrequency === 'undefined') {
  console.log(`No repeated frequency in a total of ${allFrequencyResults.length} frequencies. 🥺`);
  allFrequencyResults = checkFrequency(frequencyChanges, allFrequencyResults);
}

console.log(`There are a total of ${allFrequencyResults.length} frequencies 🤶`);
